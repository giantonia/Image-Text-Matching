...
import shutil
import yaml
from utils.dataloader import ITM_Dataset
...
def train(epoch, train_loader, network, optimizer, criterion, args, writer):
    network.train()
    ...
    for step, (images, input_ids, token_type_ids, attention_masks, labels) in enumerate(train_loader):
        ...
        labels_reshape = torch.reshape(labels, (args.batch_size, 1))
        labels_dist = labels_reshape - labels_reshape.t()
        labels_mask = (labels_dist == 0)
        labels_mask_norm = labels_mask.float() / labels_mask.float().norm(dim=1)
        i2t_pred = F.log_softmax(i2t, dim=1)
        t2i_pred = F.log_softmax(t2i, dim=1)
        i2t_loss = criterion(i2t_pred, labels_mask_norm)
        t2i_loss = criterion(t2i_pred, labels_mask_norm)
        # i2t_labels = torch.argmax(labels_mask_norm, dim=1)
        # t2i_labels = torch.argmax(labels_mask_norm.transpose(0, 1), dim=1)
        # i2t_pred = F.softmax(i2t, dim=1)
        # t2i_pred = F.softmax(t2i, dim=1)
        # i2t_loss = criterion(i2t_pred, i2t_labels)
        # t2i_loss = criterion(t2i_pred, t2i_labels)
        matching_loss = i2t_loss + t2i_loss
    ...
    for i, epoch in enumerate(range(args.epoches)):
        ...
        if epoch < epoches_list[0]:
            ...
            for param in network.module.bert.parameters():
                param.requires_grad = True
        # train for one epoch
        train_loss_100 = train(epoch, train_loader, network, optimizer, criterion, args, writer)
