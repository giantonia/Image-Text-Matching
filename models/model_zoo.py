...
class ViT_NumScore(nn.Module):
    def __init__(self):
        ...
        self.scoring_i2t = nn.Sequential(
            nn.Linear(197, 1),
            nn.ReLU()
        )
        self.scoring_t2i = nn.Sequential(
            nn.Linear(197, 1),
            nn.ReLU()
        )
    ...
    def forward(self, images_inputs, input_ids, token_type_ids, attention_masks, val=False):
        ...
        # print(torch.argmax(i2t, dim=1).cpu()==torch.tensor(range(32)))
        # print(torch.argmax(t2i, dim=1).cpu()==torch.tensor(range(32)))
        return i2t, t2i
